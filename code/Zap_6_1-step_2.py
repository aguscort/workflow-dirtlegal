...
def get_all_orders_customer(customerName):
    url = 'https://ssapi.shipstation.com/orders?customerName=' + str(customerName)  + '&sortDir=Desc&pageSize=500'
    return requests.get(url, auth=(input_data['apikey_shipstation'], input_data['password_shipstation'])).json()
...
for customer in customers:
    customer_dict.update({customer['phone'] : customer['name']})
    try:
                    ...
                    if is_Order:
                        ...
                        for task in get_tasks()["data"]:                            
                            if task["resource_type"] == "task" and str(task["name"]).find(str(target_order["orderNumber"])) != -1:
                                ...
                                break                        
                        break
            output = { "folder_name" : "Automated POA #" + target_order['orderNumber'].strip(), "text" : formatted_text, "pfd_vehicle_owner" : pfd_vehicle_owner, "pfd_vehicle_adress" : pfd_vehicle_adress, "pfd_vehicle_adress2" : pfd_vehicle_adress2, "pdf_vin" : pdf_vin, "pdf_year" : pdf_year, "pdf_make" : pdf_make, "pdf_model" : pdf_model, "wait" : str(random.randint(1, 5)), "wo" : target_order['orderNumber'].strip()}
    except:
        output = { "folder_name" : "None"}      
        
# if not is_Order and customer_dict['phone'] is not None:
#     orders =  get_all_orders_customer(customer_dict['phone'])
#     if orders['total'] > 0:
#         for order in orders['orders']:
#             for item in order['items']:
#                 if item['name'].find("Conversion") != -1:
...
#                         break    
#                 break
#         output = {"folder_name" : "#" + target_order['orderNumber'].strip()  + " " + str(name).strip(), "text" : formatted_text, "pfd_vehicle_owner" : pfd_vehicle_owner, "pfd_vehicle_adress" : pfd_vehicle_adress, "pfd_vehicle_adress2" : pfd_vehicle_adress2, "pdf_vin" : pdf_vin, "pdf_year" : pdf_year, "pdf_make" : pdf_make, "pdf_model" : pdf_model, "wait" : str(random.randint(1, 5)), "wo" : target_order['orderNumber'].strip()}    
# 
...
if is_Order:
    ...
    output = { "folder_name" : None , "text" : formatted_text, "pfd_vehicle_owner" : pfd_vehicle_owner, "pfd_vehicle_adress" : pfd_vehicle_adress, "pfd_vehicle_adress2" : pfd_vehicle_adress2, "pdf_vin" : pdf_vin, "pdf_year" : pdf_year, "pdf_make" : pdf_make, "pdf_model" : pdf_model, "wait" : str(random.randint(1, 5)), "wo" : None}        